...
class ServletsGrailsPlugin {
    ...
    def doWithDynamicMethods = { ctx ->
        ...
        def getAttributeSubscript = { String key -> delegate.getAttribute(key) }
        def setAttributeSubScript = { String key, val -> delegate.setAttribute(key, val) }
        ...
        HttpServletRequest.metaClass.getAt = { String key -> delegate.getAttribute(key) }
        // enables setting of request attributes with request["foo"] = "bar" syntax
        HttpServletRequest.metaClass.putAt = { String key, val -> delegate.setAttribute(key, val) }
        ...
        HttpServletResponse.metaClass.leftShift = { o -> delegate.writer << o }
    }
}
